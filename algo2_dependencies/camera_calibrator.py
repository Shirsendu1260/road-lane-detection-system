...
def load_pickle():
    if os.path.exists(r"algo2_dependencies\camera_calibration.p"):
        with open(r"algo2_dependencies\camera_calibration.p", "rb") as file:
        ...
        with open(r"algo2_dependencies\camera_calibration.p", "wb") as file:
...
def undistort_image(img):
    img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    undist_img = cv2.undistort(img_gray, cam_mtx, dist_coeffs, None, cam_mtx)
    return undist_img
